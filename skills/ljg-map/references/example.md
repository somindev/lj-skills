...
一个可以照着改的完整例子。frame 只写这个行业的地形怎么布（价值之河、各环节地貌、隘口、宝藏、地名）；风格、红牌金牌、李珺测量员由 `gen_illustration.py` 内置，不用写。
...
出图：算力雪山和坝上金宝藏、中游创作者村庄和内容田、峡谷红牌瓶颈、平台港镇宝藏、外海观众，李珺作测量员立山头举地图俯瞰。验收时重点检查：瓶颈和价值捕获标清、李珺认得出、整体地形易读。
...
- 生图后必 Read 验：两处标清、地名不糊、李珺认得出；不行调 frame 重生，重生前先存住当前这张。
