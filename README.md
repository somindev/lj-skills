...
| **ljg-library** | 取景框借书卡 — 一本书 → 一幅「取景框」意向画面 → 一张收藏卡（PNG）：真实封面 / 作者 / 书目 + 费曼式讲透画面；图解用 AI 生图、李珺作固定主角（从墨像参考生成），吉田诚治式绘本感风格（绘本感异世界日常、暖光斜射、治愈又精致） |
| **ljg-map** | 生态地形图卡 — 一个行业 → 一张可俯瞰的生态地形（PNG，AI 生图，默认 `-a` 动森 / 可选 `-c` cyber）：价值像河流过地貌，标出瓶颈（收窄的隘口/水坝）与价值捕获点（利润沉淀的宝藏堆），李珺作测量员俯瞰；配三个关键指标 base rate + 三个大问题 |
